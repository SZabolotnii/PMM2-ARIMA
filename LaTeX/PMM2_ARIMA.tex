...
\subsubsection{Моментно-кумулянтне Формулювання PMM2 для ARIMA}
...
\paragraph{Оцінювання моментів та кумулянтів.}
Для поточної ітерації $\boldsymbol{\theta}$ обчислюємо вибіркові кумулянти залишків
\begin{align}
...
\end{align}
Вони є оцінками моментів $\mu_r$ у формулі~\eqref{eq:pmm2_system_general}: $\hat{\mu}_2 = \hat{\kappa}_2$, $\hat{\mu}_3 = \hat{\kappa}_3$, $\hat{\mu}_4 = \hat{\kappa}_4 + 3\hat{\kappa}_2^2$. Звідси
\begin{equation}
\label{eq:delta_hat}
\hat{\Delta}(\boldsymbol{\theta}) = \hat{\mu}_2 \big(\hat{\mu}_4 - \hat{\mu}_2^2\big) - \hat{\mu}_3^2 = \hat{\kappa}_2 \left(\hat{\kappa}_4 + 2\hat{\kappa}_2^2\right) - \hat{\kappa}_3^2,
\end{equation}
...
\end{equation}
З врахуванням того, що $z_t - \eta_t = \varepsilon_t$, співвідношення~\eqref{eq:pmm2_score_arima} є прямою адаптацією багатопараметричної системи~\eqref{eq:pmm2_system_general} до ARIMA моделей. Варто підкреслити, що при $\hat{\kappa}_3 = 0$ отримаємо $s_t(\boldsymbol{\theta}) = [z_t - \eta_t(\boldsymbol{\theta})]/\hat{\kappa}_2$, і PMM2 вироджується в МНК, що узгоджується з Теоремою~\ref{thm:relative_efficiency_pmm2}.
...
\begin{itemize}
    ...
    \item \textbf{Обчислення кумулянтів та $s_t$:} $O(n)$ операцій на ітерацію.
