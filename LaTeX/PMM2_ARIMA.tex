...
\begin{enumerate}
    \item \textbf{Нестаціонарність:} оператор диференціювання $\Delta^d$ змінює дисперсійну структуру та вимагає повторного визначення інформаційної матриці.

    \item \textbf{Часова залежність:} інновації $\varepsilon_t$ не спостерігаються безпосередньо і відновлюються через рекурсію, що впливає на побудову регресорів $x_{p,t}$ у системі~\eqref{eq:pmm2_system_general}.

    \item \textbf{Багатопараметричність:} вектор параметрів $\boldsymbol{\theta} = (\phi_1,\ldots,\phi_p,\theta_1,\ldots,\theta_q)^\top$ визначає як середню, так і шумову компоненти, отже всі $Q = p+q$ поліномів $L_{2N}^{(p)}$ з підрозділу~\ref{subsec:pmm_theory} взаємопов'язані.

    \item \textbf{Ідентифікованість:} необхідно підтримувати умови стаціонарності та оборотності для того, щоб існували моменти, використані в Теоремі~\ref{thm:relative_efficiency_pmm2}.
\end{enumerate}

\subsubsection{Моментне Формулювання PMM2 для ARIMA}

Розглянемо ARIMA$(p,d,q)$ модель
\begin{equation}
\label{eq:arima_definition}
\Phi(B)(1-B)^d y_t = \Theta(B) \varepsilon_t,\qquad \varepsilon_t \sim \text{i.i.d. }(0,\sigma^2),
\end{equation}
де $B$ --- оператор зсуву ($By_t = y_{t-1}$), $\Phi(B) = 1 - \phi_1 B - \cdots - \phi_p B^p$ та $\Theta(B) = 1 + \theta_1 B + \cdots + \theta_q B^q$. Після диференціювання отримаємо стаціонарний ряд
\begin{equation}
\label{eq:differenced_series}
z_t = \Delta^d y_t,\qquad t=d+1,\ldots,T,\qquad n = T-d.
\end{equation}

Для будь-якого вектора параметрів $\boldsymbol{\theta}$ визначимо залишки як
\begin{equation}
\label{eq:residuals}
\varepsilon_t(\boldsymbol{\theta}) = z_t - \sum_{i=1}^{p} \phi_i z_{t-i} - \sum_{k=1}^{q} \theta_k \varepsilon_{t-k}(\boldsymbol{\theta}),
\end{equation}
з ініціалізаціями $\varepsilon_t = 0$ та $z_t = \bar{z}$ для $t \leq 0$, де $\bar{z} = \frac{1}{n}\sum_{t=1}^{n} z_t$.

Як показано у підрозділі~\ref{subsec:pmm_theory}, система~\eqref{eq:pmm2_system_general} будується через регресори $x_{p,t}$, що в лінійній моделі збігаються з елементами матриці $\boldsymbol{X}$. Для ARIMA моделі детермінована частина
\begin{equation}
\label{eq:eta_definition}
\eta_t(\boldsymbol{\theta}) = \mathbb{E}\left[z_t \mid \mathcal{F}_{t-1}\right] = \sum_{i=1}^{p} \phi_i z_{t-i} + \sum_{k=1}^{q} \theta_k \varepsilon_{t-k}(\boldsymbol{\theta})
\end{equation}
призводить до визначення ``псевдорегресорів''
\begin{equation}
\label{eq:x_definition}
x_{p,t}(\boldsymbol{\theta}) = \frac{\partial \eta_t(\boldsymbol{\theta})}{\partial \theta_p} = -\,\frac{\partial \varepsilon_t(\boldsymbol{\theta})}{\partial \theta_p},
\end{equation}
які є прямою адаптацією конструкції з~\eqref{eq:vector_estimation}. Рекурсії для цих величин відрізняються для AR та MA параметрів.

\paragraph{AR параметри.}
Для $j=1,\ldots,p$:
\begin{equation}
\label{eq:x_phi_recursion}
x_{\phi_j,t} = z_{t-j} + \sum_{k=1}^{q} \theta_k x_{\phi_j,t-k},\qquad x_{\phi_j,t} = 0 \text{ для } t \leq 0.
\end{equation}

\paragraph{MA параметри.}
Використовуючи $\varepsilon_{t} = z_t - \eta_t$, маємо для $j=1,\ldots,q$:
\begin{equation}
\label{eq:x_theta_recursion}
x_{\theta_j,t} = \varepsilon_{t-j}(\boldsymbol{\theta}) + \sum_{k=1}^{q} \theta_k x_{\theta_j,t-k},\qquad x_{\theta_j,t} = 0 \text{ для } t \leq 0.
\end{equation}
Похідні залишків, які будуть потрібні при чисельному розв'язанні, отримуємо безпосередньо з~\eqref{eq:x_definition}:
\begin{equation}
\label{eq:residual_derivative}
\frac{\partial \varepsilon_t(\boldsymbol{\theta})}{\partial \theta_p} = -x_{p,t}(\boldsymbol{\theta}).
\end{equation}

\paragraph{Оцінювання центральних моментів.}
Для поточної ітерації $\boldsymbol{\theta}$ обчислюємо вибіркові центральні моменти залишків
\begin{align}
\hat{\mu}_2(\boldsymbol{\theta}) &= \frac{1}{n} \sum_{t=1}^{n} \varepsilon_t^2(\boldsymbol{\theta}), \label{eq:sample_m2} \\
\hat{\mu}_3(\boldsymbol{\theta}) &= \frac{1}{n} \sum_{t=1}^{n} \varepsilon_t^3(\boldsymbol{\theta}), \label{eq:sample_m3} \\
\hat{\mu}_4(\boldsymbol{\theta}) &= \frac{1}{n} \sum_{t=1}^{n} \varepsilon_t^4(\boldsymbol{\theta}). \label{eq:sample_m4}
\end{align}
Звідси
\begin{equation}
\label{eq:delta_hat}
\hat{\Delta}(\boldsymbol{\theta}) = \hat{\mu}_2 \big(\hat{\mu}_4 - \hat{\mu}_2^2\big) - \hat{\mu}_3^2,
\end{equation}
що є емпіричним аналогом~\eqref{eq:delta_definition}.

\paragraph{Стохастичний поліном другого порядку.}
Згідно з~\eqref{eq:pmm2_quadratic_form}, $p$-та рівноважна умова для ARIMA набуває вигляду
\begin{equation}
\label{eq:pmm2_score_arima}
g_p(\boldsymbol{\theta}) = \sum_{t=1}^{n} x_{p,t}(\boldsymbol{\theta})\, s_t(\boldsymbol{\theta}) = 0,
\end{equation}
де
\begin{equation}
\label{eq:score_inner_term}
s_t(\boldsymbol{\theta}) = \frac{\hat{\mu}_4 - \hat{\mu}_2^2 + 2\hat{\mu}_3 \eta_t(\boldsymbol{\theta})}{\hat{\Delta}} \big[z_t - \eta_t(\boldsymbol{\theta})\big] - \frac{\hat{\mu}_3}{\hat{\Delta}} \left[z_t^2 - \eta_t^2(\boldsymbol{\theta}) - \hat{\mu}_2 \right].
\end{equation}
З врахуванням того, що $z_t - \eta_t = \varepsilon_t$, співвідношення~\eqref{eq:pmm2_score_arima} є прямою адаптацією багатопараметричної системи~\eqref{eq:pmm2_system_general} до ARIMA моделей. Варто підкреслити, що при $\hat{\mu}_3 = 0$ отримаємо $s_t(\boldsymbol{\theta}) = [z_t - \eta_t(\boldsymbol{\theta})]/\hat{\mu}_2$, і PMM2 вироджується в МНК, що узгоджується з Теоремою~\ref{thm:relative_efficiency_pmm2}.

\subsubsection{Чисельне Розв'язання Системи PMM2}

Побудуємо векторну функцію
\begin{equation}
\label{eq:vector_score}
\mathbf{g}(\boldsymbol{\theta}) = \big(g_1(\boldsymbol{\theta}),\ldots,g_{p+q}(\boldsymbol{\theta})\big)^\top,
\end{equation}
яка є градієнтом стохастичного полінома за параметрами. Знаходження $\hat{\boldsymbol{\theta}}_{\text{PMM2}}$ зводиться до розв'язання $\mathbf{g}(\boldsymbol{\theta}) = \mathbf{0}$. Метод Ньютона-Рафсона використовує матрицю Якобі
\begin{equation}
\label{eq:jacobian_definition}
J_{p\ell}(\boldsymbol{\theta}) = \frac{\partial g_p(\boldsymbol{\theta})}{\partial \theta_\ell} = \sum_{t=1}^{n} \left[ \frac{\partial x_{p,t}}{\partial \theta_\ell} s_t(\boldsymbol{\theta}) + x_{p,t}(\boldsymbol{\theta}) \frac{\partial s_t(\boldsymbol{\theta})}{\partial \theta_\ell} \right],
\end{equation}
де перша складова відображає взаємозалежність ``псевдорегресорів'', а друга --- чутливість стохастичного полінома до зміни моментів.

Диференціюючи рекурсії~\eqref{eq:x_phi_recursion}--\eqref{eq:x_theta_recursion}, отримуємо допоміжні співвідношення: похідні $x_{\phi_j,t}$ за AR-параметрами повторюють структуру~\eqref{eq:x_phi_recursion}, а похідні за MA-параметрами містять додаткові доданки $x_{\phi_j,t-m}$ (для всіх $m=1,\ldots,q$), що враховують залежність від коефіцієнтів $\theta_m$. Аналогічно, похідні $x_{\theta_j,t}$ включають похідні залишків (через~\eqref{eq:residual_derivative}) та ковзні ланцюги. Вирази для $\partial s_t / \partial \theta_\ell$ містять похідні моментів:
\begin{equation}
\label{eq:ds_term}
\frac{\partial s_t(\boldsymbol{\theta})}{\partial \theta_\ell} = \frac{\partial }{\partial \theta_\ell} \left[ \frac{\hat{\mu}_4 - \hat{\mu}_2^2 + 2\hat{\mu}_3 \eta_t}{\hat{\Delta}} \varepsilon_t - \frac{\hat{\mu}_3}{\hat{\Delta}} \left(2 \eta_t \varepsilon_t + \varepsilon_t^2 - \hat{\mu}_2\right) \right],
\end{equation}
які обчислюються через ланцюгове правило та рекурсії~\eqref{eq:residual_derivative}. На практиці, для стабілізації алгоритму доцільно використовувати симетризовану апроксимацію Якобіана (наприклад, квазі-Ньютон BFGS), що коректно враховує структуру~\eqref{eq:jacobian_definition}.
...
\REPEAT
    ...
    \STATE \textit{3.2. Обчислити центральні моменти} $\hat{\mu}_2, \hat{\mu}_3, \hat{\mu}_4$ за формулами~\eqref{eq:sample_m2}--\eqref{eq:sample_m4}

    \STATE \textit{3.3. Побудувати псевдорегресори} $x_{p,t}(\boldsymbol{\theta}^{(k-1)})$ за рекурсіями~\eqref{eq:x_phi_recursion}--\eqref{eq:x_theta_recursion}

    \STATE \textit{3.4. Обчислити внутрішні оцінки} $\eta_t = z_t - \varepsilon_t$ та $s_t(\boldsymbol{\theta}^{(k-1)})$ за формулою~\eqref{eq:score_inner_term}

    \STATE \textit{3.5. Обчислити вектор} $\mathbf{g}^{(k)}$ за~\eqref{eq:vector_score} (компоненти~\eqref{eq:pmm2_score_arima})

    \STATE \textit{3.6. Обчислити або апроксимувати Якобіан} $\mathbf{J}^{(k)}$ відповідно до~\eqref{eq:jacobian_definition}

    \STATE \textit{3.7. Оновити параметри:} розв'язати $\mathbf{J}^{(k)} \Delta \boldsymbol{\theta}^{(k)} = \mathbf{g}^{(k)}$ та покласти $\boldsymbol{\theta}^{(k)} \leftarrow \boldsymbol{\theta}^{(k-1)} - \Delta \boldsymbol{\theta}^{(k)}$

    \STATE \textit{3.8. Перевірити обмеження:} Якщо $\boldsymbol{\theta}^{(k)}$ порушує стаціонарність/оборотність, проектувати на допустимий простір
...
\STATE Обчислити оцінку дисперсії: $\hat{\sigma}^2 \leftarrow \hat{\mu}_2(\hat{\boldsymbol{\theta}}_{\text{PMM2}})$
...
\begin{itemize}
    ...
    \item \textbf{Обчислення моментів та $s_t$:} $O(n)$ операцій на ітерацію.

    \item \textbf{Формування $\mathbf{g}$:} $O(nk)$ операцій.

    \item \textbf{Оцінювання Якобіана:} $O(nk^2)$ операцій (або $O(k^2)$ для квазі-Ньютона).

    \item \textbf{Розв'язання лінійної системи:} $O(k^3)$ операцій.

    \item \textbf{Загальна складність:} $O(I \cdot nk^2)$, де $I$ --- кількість ітерацій (типово $I = 10$--$50$).
\end{itemize}

Порівняно з класичним MLE, PMM2 має подібну обчислювальну складність, оскільки обидва методи потребують ітеративної оптимізації з формуванням векторів умов та їхніх Якобіанів.

\subsection{Асимптотичні Властивості PMM2 для ARIMA}
\label{subsec:asymptotic_theory}

Для аналізу асимптотики перепишемо систему умов~\eqref{eq:pmm2_score_arima} у вигляді усередненого вектору моментних рівнянь. Нехай $k = p+q$ --- кількість параметрів та
\begin{equation}
\label{eq:gn_definition}
\mathbf{g}_n(\boldsymbol{\theta}) = \frac{1}{n} \sum_{t=1}^{n} \boldsymbol{\psi}_t(\boldsymbol{\theta}), \qquad \boldsymbol{\psi}_t(\boldsymbol{\theta}) = \big(x_{1,t}(\boldsymbol{\theta}) s_t(\boldsymbol{\theta}), \ldots, x_{k,t}(\boldsymbol{\theta}) s_t(\boldsymbol{\theta})\big)^\top.
\end{equation}
Тоді оцінювач PMM2 визначається умовою $\mathbf{g}_n(\hat{\boldsymbol{\theta}}_{\text{PMM2}}) = \mathbf{0}$, тобто є Z-оцінкою у сенсі Newey \& McFadden (1994).
...
\begin{enumerate}
    \item Показати, що $\mathbf{g}_n(\boldsymbol{\theta})$ збігається рівномірно до $\mathbf{g}(\boldsymbol{\theta}) = \mathbb{E}[\boldsymbol{\psi}_t(\boldsymbol{\theta})]$ за законом великих чисел для ергодичних процесів.

    \item Показати, що система моментних рівнянь $\mathbf{g}(\boldsymbol{\theta}) = \mathbf{0}$ має єдиний розв'язок у $\boldsymbol{\theta}_0$ (умова ідентифікації).

    \item Застосувати теорему 2.1 з White (1994) або теорему 7.1 з Newey \& McFadden (1994) для Z-оцінок з залежними даними.
\end{enumerate}
...
з матрицями:
\begin{align}
\mathbf{A} &= \mathbb{E}\left[ \frac{\partial \boldsymbol{\psi}_t(\boldsymbol{\theta}_0)}{\partial \boldsymbol{\theta}^\top} \right] \label{eq:matrix_A} \\
\mathbf{B} &= \mathbb{E}\left[ \boldsymbol{\psi}_t(\boldsymbol{\theta}_0)\, \boldsymbol{\psi}_t(\boldsymbol{\theta}_0)^\top \right] \label{eq:matrix_B}
\end{align}
\end{theorem}
...
\end{equation}
де $\hat{\mathbf{A}} = \frac{1}{n} \sum_{t=1}^{n} \frac{\partial \boldsymbol{\psi}_t(\hat{\boldsymbol{\theta}}_{\text{PMM2}})}{\partial \boldsymbol{\theta}^\top}$ та $\hat{\mathbf{B}} = \frac{1}{n} \sum_{t=1}^{n} \boldsymbol{\psi}_t(\hat{\boldsymbol{\theta}}_{\text{PMM2}})\, \boldsymbol{\psi}_t(\hat{\boldsymbol{\theta}}_{\text{PMM2}})^\top$ --- вибіркові аналоги матриць~\eqref{eq:matrix_A}--\eqref{eq:matrix_B}.
...
В асимптотичному випадку із ростом обсягу вібірок, відносна ефективність добре апроксимується формулою~\eqref{eq:re_pmm2_ols}.
