...
\date{
\vspace{-1em}
{\small Received 30 November 2023; Accepted 6 September 2024} \\[0.5em]
{\small Correspondence to: Сергій Заболотній, Cherkasy State Business College, Cherkasy, Ukraine.} \\
...
\noindent{\small \textbf{Keywords:} ARIMA models; polynomial maximization method; non-Gaussian innovations; parameter estimation; asymptotic efficiency; time series analysis; skewed distributions; Monte Carlo simulation.}
...
\subsection{Класичні Підходи: Короткий Огляд}
...
\subsection{Глибинне Навчання для Часових Рядів}
...
Розглянемо застосування стохастичних поліномів до лінійної багатофакторної регресії з асиметрично розподіленою випадковою складовою~\cite{zabolotnii2018polynomial}. Як базисні функції використовуються степеневі перетворення, математичними сподіваннями від яких є моменти відповідного порядку. Нехай спостереження $\{y_v\}_{v=1}^N$ описуються моделлю
\begin{equation}
...
\end{equation}
де $\gamma_3$ та $\gamma_4$ --- стандартизовані коефіцієнти асиметрії та ексцесу інновацій відповідно, а $\RE > 1$ означає виграш PMM2 у точності відносно OLS.
\end{theorem}

\begin{proof}[Ескіз доведення]
Доведення базується на співвідношенні варіаційних матриць:
\begin{equation}
\mathbf{V}_{\text{PMM2}} = \frac{1}{\RE_{\text{PMM2/OLS}}} \cdot \mathbf{V}_{\text{OLS}}
\end{equation}
...
\end{equation}
Відношення інформацій $J_{2N}/J_{1N}$ дає коефіцієнт відносної ефективності, а перехід від моментного до кумулянтного опису дає формулу~\eqref{eq:re_pmm2_ols}. Детальне доведення наведено в~\cite{kunchenko2002polynomial}.
\end{proof}

\paragraph{Інтерпретація відносної ефективності.}

\begin{itemize}
    \item Для гаусових інновацій ($\gamma_3 = 0$, $\gamma_4 = 0$): $\RE = 1$, тобто PMM еквівалентний OLS, оскільки оцінки співпадають.

    \item Для асиметричних розподілів ($\gamma_3 \neq 0$): $\RE > 1$, що означає виграш PMM2 у точності. Наприклад, для $\gamma_3 = 1.5$ та $\gamma_4 = 3$: $\RE \approx 1.82$, що відповідає зменшенню дисперсії PMM2 на 45\% порівняно з OLS.

    \item При використанні PMM2 відносний виграш у точності залежить лише від коефіцієнтів асиметрії $\gamma_3$ та ексцесу $\gamma_4$.
...
\textbf{Практичне зауваження.} Для скінченних вибірок ($N < 500$) можливі малі відхилення через неточність першокрокового оцінювання. У таких випадках рекомендується використовувати HAC (heteroskedasticity and autocorrelation consistent) стандартні похибки типу Newey-West для робастності.
...
\subsubsection{Bootstrap Довірчі Інтервали}
\label{subsubsec:bootstrap_ci}

Для кількісної оцінки невизначеності Monte Carlo оцінок обчислено 95\% довірчі інтервали методом bias-corrected and accelerated (BCa) bootstrap~\cite{efron1993introduction, diciccio1996bootstrap}:

\paragraph{Метод та Конфігурація.}
\begin{itemize}
    \item \textbf{Кількість реплікацій}: 1,000 bootstrap вибірок для кожної метрики
    \item \textbf{Рівень довіри}: 95\%
    \item \textbf{Метод}: BCa (коригування на зміщення та асиметрію)
    \item \textbf{Fallback стратегія}: Percentile метод при збої BCa, нормальне наближення при збої обох
\end{itemize}

\paragraph{Обчислювальна процедура.}
Для кожної комбінації (модель, розподіл, N, метод, параметр):
\begin{enumerate}
    \item Зберігаємо вихідні оцінки $\{\hat{\theta}_{j,M}^{(r)}\}_{r=1}^{R}$ з Monte Carlo симуляцій
    \item Для кожної метрики $\psi$ (bias, variance, MSE): створюємо $B = 1000$ bootstrap вибірок з повторенням, обчислюємо $\psi^{*(b)}$ для кожної вибірки $b = 1, \ldots, B$, застосовуємо BCa коригування для асиметрії та зміщення, отримуємо довірчий інтервал $[\psi_{\text{lower}}, \psi_{\text{upper}}]$
    \item Обчислюємо стандартну помилку: $SE(\psi) = \text{sd}(\{\psi^{*(b)}\}_{b=1}^B)$
\end{enumerate}

\paragraph{Успішність та Валідація.}
\begin{itemize}
    \item \textbf{Успішність обчислення CI}: 93.8\% для bias, variance, MSE
    \item \textbf{Властивості CI}: Ширина зменшується з $N$ (підтверджує $\sqrt{N}$ закон)
    \item \textbf{Перевірка покриття}: CI містять точкові оцінки; асиметричні для сильно скошених розподілів
\end{itemize}

\paragraph{Інтерпретація.}
Довірчі інтервали дозволяють кількісно оцінити статистичну значимість переваг PMM2. Наприклад, для Gamma(2,1) інновацій при $N=500$:
\begin{itemize}
    \item CSS MSE: $0.00106$ [95\% CI: $0.00100$, $0.00113$]
    \item PMM2 MSE: $0.00061$ [95\% CI: $0.00057$, $0.00065$]
\end{itemize}
Непересічність довірчих інтервалів підтверджує статистично значуще зменшення MSE на 42.5\%.
...
\textbf{Висновки:}
\begin{itemize}
    \item Для гаусових інновацій PMM2 та Hubерівська M-оцінка (M-EST) демонструють відносну ефективність $\RE \approx 1.02 \pm 0.02$, що практично дорівнює 1 і збігається з теоретичною нейтральністю методу за нормального шуму.
...
\begin{table}[h]
\centering
\caption{Результати Monte Carlo для ARIMA(1,1,0), $\phi_1 = 0.7$, Gamma(2,1) інновації ($\gamma_3 \approx 1.41$) з bootstrap 95\% довірчими інтервалами}
\label{tab:arima110_gamma}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{N} & \textbf{Метод} & \textbf{Bias [95\% CI]} & \textbf{MSE [95\% CI]} & \textbf{RE} \\
\midrule
\multirow{2}{*}{100} & CSS  & $-0.0153$ [$-0.0185$, $-0.0119$] & $0.00566$ [$0.00524$, $0.00609$] & 1.00 \\
                     & PMM2 & $-0.0024$ [$-0.0050$, $0.0004$] & $0.00352$ [$0.00330$, $0.00376$] & 1.61 \\
\midrule
\multirow{2}{*}{200} & CSS  & $-0.0056$ [$-0.0078$, $-0.0034$] & $0.00269$ [$0.00252$, $0.00290$] & 1.00 \\
                     & PMM2 & $-0.0000$ [$-0.0018$, $0.0018$] & $0.00163$ [$0.00152$, $0.00174$] & 1.65 \\
\midrule
\multirow{2}{*}{500} & CSS  & $-0.0031$ [$-0.0044$, $-0.0016$] & $0.00106$ [$0.00100$, $0.00113$] & 1.00 \\
                     & PMM2 & $-0.0003$ [$-0.0015$, $0.0007$] & $0.00061$ [$0.00057$, $0.00065$] & 1.75 \\
\midrule
\multirow{2}{*}{1000} & CSS  & $-0.0014$ [$-0.0024$, $-0.0005$] & $0.00050$ [$0.00047$, $0.00053$] & 1.00 \\
                      & PMM2 & $0.0002$ [$-0.0007$, $0.0012$] & $0.00030$ [$0.00028$, $0.00032$] & 1.63 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Висновки:}
\begin{itemize}
    \item PMM2 демонструє суттєве покращення для gamma інновацій: RE $\approx$ 1.61--1.75.
    \item \textbf{Статистична значимість}: Bootstrap 95\% довірчі інтервали для MSE повністю непересічні між CSS та PMM2, що підтверджує статистично значуще покращення.
    \item \textbf{Незміщеність PMM2}: Всі bias довірчі інтервали містять нуль (наприклад, при $N=200$: [$-0.0018$, $0.0018$]).
    \item При $N = 500$, PMM2 досягає MSE $=0.00061$ [$0.00057$, $0.00065$] проти CSS MSE $=0.00106$ [$0.00100$, $0.00113$] --- зменшення на 42.5\%.
...
\item \textbf{Відсутність втрати ефективності для гаусових інновацій:} За нормального шуму обидва методи (PMM2 і M-EST) дають $\RE = 1.02 \pm 0.02$, тобто статистично не відрізняються від класичних оцінок.
...
\begin{enumerate}
    ...
    \item \textbf{Застереження щодо AIC/BIC для PMM2.} Оскільки PMM2 не є методом максимальної правдоподібності, стандартне обчислення AIC/BIC через \texttt{stats::arima} може бути некоректним. У даній роботі AIC/BIC обчислено через \textit{post-hoc} гаусову лог-правдоподібність для порівняння, але ці критерії мають розглядатися як орієнтовні. Для коректного порівняння моделей рекомендується використовувати: (1) out-of-sample метрики (RMSE, MAE, CRPS), (2) cross-validation, або (3) квазі-інформаційні критерії типу QIC~\cite{pan2001akaike}, що базуються на інформації Годамбара. У нашому випадку переваги PMM2 підтверджуються також RMSE/MAE, що є незалежними від припущення про правдоподібність.
...
Ефективність PMM2 визначається коефіцієнтами асиметрії $\gamma_3$ та ексцесу $\gamma_4$ залишків ~\eqref{eq:re_pmm2_ols}, а порогові значення для вибору методу узагальнені в Табл.~\ref{tab:wti_practical_recommendations}. Розгорнуте decision tree та секторні приклади перенесено до Додатку~\ref{app:wti_guidelines}.
...
\label{subsec:wti_out_of_sample}

Для оцінки узагальнюючої здатності PMM2 за межами навчальної вибірки застосовано два підходи: фіксований розподіл (80/20) та ковзне вікно (rolling window).
...
\begin{itemize}
    \item PMM2 демонструє суттєве покращення для специфікацій AR: для ARIMA(1,1,0) зменшення RMSE становить 38\% (фіксований) та 11\% (rolling window).
    \item Ковзне вікно (1,094 прогнозів) підтверджує стабільність переваг PMM2 у реалістичному сценарії динамічного прогнозування.
    \item Для моделей з MA компонентами (ARIMA(0,1,1), ARIMA(1,1,1)) обидва методи показують схожу прогнозну якість.
\end{itemize}

\subsection{Діагностичні Тести з P-значеннями}
\label{subsec:wti_diagnostics}

Застосовано чотири статистичні тести для оцінки адекватності моделей та властивостей залишків (Табл.~\ref{tab:wti_diagnostics}).
...
\textbf{Інтерпретація:}
\begin{itemize}
    \item \textbf{Значуща негаусовість} (Jarque-Bera $p < 0.001$, Shapiro-Wilk $p < 0.001$) підтверджує мотивацію для використання PMM2, який експліцитно враховує кумулянти вищих порядків.
    \item \textbf{Автокореляція} (Ljung-Box $p < 0.001$) вказує на можливу недоспецифікацію моделі ARIMA(1,1,1); моделі вищого порядку або GARCH-розширення можуть покращити діагностику.
    \item \textbf{Гетероскедастичність} (ARCH тест $p < 0.001$) свідчить про часово-змінну дисперсію; майбутні дослідження можуть розглянути GARCH-PMM2 гібриди.
    \item PMM2 демонструє дещо нижчий ARCH-ефект (F = 16.19 vs 17.77), що вказує на кращу адаптацію до негаусовості.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Out-of-sample валідація} підтверджує практичну цінність PMM2: 11--38\% зменшення RMSE для AR специфікацій.
    \item \textbf{Діагностичні тести} засвідчують негаусовість залишків ($p < 0.001$), що валідує теоретичне обґрунтування PMM2.
